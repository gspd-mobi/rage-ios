...
class ParamsBuilderSpec: QuickSpec {
    ...
    override func spec() {
        describe("params builder") {
            ...
            describe("url builder from params") {
            ...
            describe("url encoding from field params") {
                it("can build url encoded string with no field parameters") {
                    let encodedString = builder.buildUrlEncodedString([:])
                    expect(encodedString).to(equal(""))
                }

                it("can build url encoded string with 1 field parameter") {
                    let encodedString = builder.buildUrlEncodedString(["username": FieldParameter(value: "paul_k")])
                ...
                it("can build url encoded string with 1 encoded field parameter") {
                    let encodedString = builder.buildUrlEncodedString(["password": FieldParameter(value: "pa  word", encoded: true)])
